...
func TestReElection(t *testing.T) {
	...
	fmt.Println("[Test]: leader1 recover!");
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	fmt.Println("[Test]: no quorum!");
	...
	fmt.Println("[Test]: has quorum!");
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	fmt.Println("[Test]: rejoin node!");
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestBasicAgree(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		fmt.Printf("TestBasicAgree one go....\n")
...
loop:
	for try := 0; try < 5; try++ {
		...
		leader := cfg.checkOneLeader()
		fmt.Printf("[test]:cfg.rafts[leader].Start(1)\n")
		...
		for ii := 0; ii < iters; ii++ {
			...
			go func(i int) {
				defer wg.Done()
				fmt.Printf("[test]:cfg.rafts[leader].Start(100 + i)= %d\n",100+i)
		...
		cmds := []int{}
		for index := range is {
			fmt.Printf("[test]:cfg.Wait(%d,%d,%d)\n",index, servers, term)
...
func TestRejoin(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	fmt.Printf("==========[Test]: leader disconnect!\n")
	...
	fmt.Printf("==========[Test]: new leader disconnect!\n")
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)

	// old leader connected again
	fmt.Printf("==========[Test]: old leader rejoined!\n")
	cfg.connect(leader1)

	cfg.one(104, 2)

	// all together now
	fmt.Printf("==========[Test]: new leader rejoined!\n")
	cfg.connect(leader2)
...
func TestBackup(t *testing.T) {
	...
	fmt.Printf("==========[Test]: put leader and one follower in a partition!\n")
	leader1 := cfg.checkOneLeader()
	...
	fmt.Printf("==========[Test]: all disconnect then recover other partition!\n")
	...
	fmt.Printf("==========[Test]: leader1 + 2 disconnect!\n")
	leader2 := cfg.checkOneLeader()
...
func TestPersist1(t *testing.T) {
	...
	fmt.Printf("==========[Test]: start1 and disconnect->connect!\n")
	for i := 0; i < servers; i++ {
		cfg.start1(i)
	}
	for i := 0; i < servers; i++ {
	...
	fmt.Printf("==========[Test]: leader1 and disconnect-restart->connect!\n")
	...
	fmt.Printf("==========[Test]: leader2 and disconnect-restart->connect!\n")
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
...
func TestUnreliableAgree(t *testing.T) {
	...
	fmt.Printf("==========[Test]: setunreliable false!\n")
...
func TestFigure8Unreliable(t *testing.T) {
	...
	fmt.Printf("==========[Test]: cfg.one(rand.Int()%%10000, servers)!\n")
