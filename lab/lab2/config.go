...
func (cfg *config) start1(i int) {
	...
	go func() {
		for m := range applyCh {	// 循环读取channel ch关闭时，for循环会自动结束
			err_msg := ""
			if m.UseSnapshot {
				// ignore the snapshot
				...
				for j := 0; j < len(cfg.logs); j++ {
					//fmt.Printf("ok,from applyCh: ix:%d j:%d od:%d\n",m.Index,j,cfg.logs[j][m.Index])
				...
				cfg.mu.Unlock()
				fmt.Printf("ok,from applyCh! cfg.logs[%d][%d] =%d\n",i,m.Index,v)
		...
		fmt.Println("listener applyCh end")
	}()
